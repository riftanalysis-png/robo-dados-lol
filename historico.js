...
// Freio para respeitar o Rate Limit da GRID (Closed Platform: 40 req/min = 1.5s)
...
function formatTournamentDetails(campeonato, tipo) {
  if (tipo === "SCRIM" || campeonato.toUpperCase().includes("SCRIM")) {
    return { game_type: "SCRIM", split: "N/A" };
  }
  ...
  if (campeonato.includes(" - ")) {
    ...
    if (rest.includes("(")) split = rest.split("(")[0].trim(); 
    else split = rest;
  } else if (campeonato.includes("(")) {
    game_type = campeonato.split("(")[0].trim();
  }
  split = split.replace(/\b20\d{2}\b/g, '').trim();
  split = split.replace(/\s{2,}/g, ' ');
...
function extrairDadosPorTempo(endStateDetails, temposMinutos) {
  ...
  for (const tempoMs of temposMs) {
    ...
    for (const [id, frameData] of Object.entries(frame.participantFrames)) {
      dados[id] = { 
          cs: (frameData.minionsKilled || 0) + (frameData.jungleMinionsKilled || 0), 
          xp: frameData.xp || 0, 
          gold: frameData.totalGold || 0 
      };
    }
...
function extrairEstatisticasDB(jogo, detailsJson, eventLines, matchId, teamMap, patch) {
  ...
  return jogo.participants.map(p => {
    ...
    // Estatísticas até os 12 minutos extraídas de eventLines (Mais seguro)
    ...
    return {
      ...
      cs_6: dadosPorTempo[6]?.[pId]?.cs || 0, 
      cs_12: dadosPorTempo[12]?.[pId]?.cs || 0, 
      cs_18: dadosPorTempo[18]?.[pId]?.cs || 0, 
      xp_12: dadosPorTempo[12]?.[pId]?.xp || 0, 
      gold_12: dadosPorTempo[12]?.[pId]?.gold || 0,
      vpm_at_12: toNum(wards12 / 12),
      kda_at_12: deaths12 > 0 ? toNum((kills12 + assists12) / deaths12) : kills12 + assists12,
...
async function baixarESepararZip(url) {
  try {
    ...
    // AVISO CLARO SE A CHAVE FOR BLOQUEADA PELA API
    console.log(`❌ Erro ao baixar ${url}: ${e.response?.status || e.message}`);
    return [];
  }
}
...
async function processarPartidas(partidas) {
  for (const partida of partidas) {
    ...
    await delay(1500); // 🛑 Freio de 1.5s

    const detalhesRiot = await baixarESepararZip(`https://api.grid.gg/file-download/end-state-details/riot/series/${seriesId}`);
    await delay(1500); // 🛑 Freio de 1.5s
    ...
    for (const arq of endStatesRiot) {
        ...
        // Regex blindada: pega o último número antes de ".json"
        ...
        if (lado_vencedor === 'unknown' && endState.participants) {
            const winPlayer = endState.participants.find(p => p.win === true || p.win === "Win");
            if (winPlayer) lado_vencedor = (winPlayer.teamId || winPlayer.teamID) === 100 ? 'blue' : 'red';
        }
        if (lado_vencedor === 'unknown' && gridState && gridState.seriesState && gridState.seriesState.games) {
            const jogoNoGrid = gridState.seriesState.games[parseInt(gameNum) - 1];
            if (jogoNoGrid && jogoNoGrid.teams) {
                const winGrid = jogoNoGrid.teams.find(t => t.won === true || t.win === true);
                if (winGrid && winGrid.side) lado_vencedor = winGrid.side.toLowerCase();
            }
        }
        if (lado_vencedor === 'unknown') {
            console.log(`⚠️ Jogo ${realMatchId} sem vencedor. Jogado fora!`);
            continue;
        }

        // Busca de eventos e detalhes protegida por regex
        let arqEvento = eventosRiot.find(a => a.nome.match(new RegExp(`_${gameNum}\\.jsonl?$`)));
        if (!arqEvento && eventosRiot.length > 0) arqEvento = eventosRiot[0];
        const eventLines = arqEvento ? arqEvento.conteudo.trim().split('\n').filter(l => l).map(l => JSON.parse(l)) : [];

        let arqDetalhes = detalhesRiot.find(a => a.nome.match(new RegExp(`_${gameNum}\\.json$`)));
        if (!arqDetalhes && detalhesRiot.length > 0) arqDetalhes = detalhesRiot[0];
        const detailsJson = arqDetalhes ? JSON.parse(arqDetalhes.conteudo) : null;
        
        if (!detailsJson) {
            console.log(`⚠️ ALERTA: O jogo ${realMatchId} não tem arquivo Details (Frames). Os status de gold/cs/xp de 12 min ficarão zerados.`);
        }
        ...
        const { game_type, split } = formatTournamentDetails(partida.campeonato, partida.tipo);

        const serieData = { id: seriesId, description: `${teamMap[100] || 'BLUE'} vs ${teamMap[200] || 'RED'} - ${game_type}` };
...
async function executarBackfillStreaming(tipo) {
  ...
  while (hasNextPage) {
    ...
    try {
      ...
      const partidasDaPagina = data.edges.map(e => ({ id: e.node.id, campeonato: e.node.tournament?.name || tipo, tipo: tipo }));
      let partidasValidas = [];
