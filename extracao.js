...
// Usamos a Axios para os dois endpoints: Central (Paginação) e File Download (Zips)
const gridApiGraphQL = axios.create({ 
  baseURL: 'https://api.grid.gg/central-data/graphql',
  headers: { 'x-api-key': process.env.API_KEY, 'Content-Type': 'application/json' } 
});

const gridApiFiles = axios.create({ 
  baseURL: 'https://api.grid.gg/file-download',
...
function formatTournamentDetails(campeonato, tipo) {
  if (tipo === "SCRIM" || campeonato.toUpperCase().includes("SCRIM")) return { game_type: "SCRIM", split: "N/A" };
  let game_type = campeonato, split = "";
...
function extrairEstatisticasDB(jogo, detailsJson, eventLines, matchId, teamMap, patch) {
  ...
  return jogo.participants.map(p => {
    ...
    return {
      ...
      cs_6: dadosPorTempo[6]?.[pId]?.cs || 0, cs_12: dadosPorTempo[12]?.[pId]?.cs || 0, cs_18: dadosPorTempo[18]?.[pId]?.cs || 0, 
...
// --- 🚀 O NOVO MOTOR QUE NÃO VAZA MEMÓRIA ---

// Esta função retorna a instância do ZIP crua na memória, em vez de Strings descompactadas.
async function baixarZIPSeguro(url) {
  try {
    ...
    return { isFile: true, content: buffer.toString('utf8') };
  } catch (e) {
    console.log(`❌ Erro no Grid API: ${url} - ${e.message}`);
    return null;
  }
}
...
async function processarPartidas(partidas) {
  for (const partida of partidas) {
    ...
    if (!zipRiot || zipRiot.isFile) {
        console.log(`⚠️ Série ${seriesId} não tem arquivo ZIP válido da Riot. Pulando.`);
        continue;
    }

    // Lemos apenas os "índices" dos arquivos, não os convertemos para texto ainda!
    ...
    for (const entrada of entradasRiot) {
        ...
        console.log(`▶️ Extraindo da RAM e processando Jogo ${gameNum} (ID: ${realMatchId})...`);

        // Extrai apenas o JSON gigantesco deste jogo específico, usa, e depois o próprio JavaScript deleta no fim do Loop!
        ...
        // Abre os Details apenas deste jogo específico
        ...
        const { error: errMatch } = await supabase.from('matches').upsert({
            ...
            game_start_time: new Date(timestamp).toISOString(), game_type: game_type, split: split,
            blue_team_tag: String(teamMap[100]), red_team_tag: String(teamMap[200]), winner_side: lado_vencedor
        }, { onConflict: 'id' });
        ...
        if (statsFinais.length > 0) await supabase.from('player_stats_detailed').insert(statsFinais);
